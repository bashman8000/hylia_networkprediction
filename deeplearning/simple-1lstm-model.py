...
def main():
	df_aofa_lond = pd.read_csv('oneyear/sacr_sunnout.csv', header=None)
	...
	#try:
	#dates = df_aofa_lond['Time'].apply(lambda x: datetime.strptime(x, '%m/%d/%y %H:%M'))
	...
	hist=model.fit(X_train_linkdata, Y_train_linkdata, epochs=EPCH, batch_size=batch_size, 
	shuffle=False, validation_data = (X_val_linkdata, Y_val_linkdata), verbose = 0, callbacks=[time_callback])
